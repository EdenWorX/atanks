...
bool Load_Game() {
			...
			if ( !strcasecmp ( line, "GLOBAL" ) )
				...
				switch ( stage ) {
					case SGS_ENVIRONMENT:
						...
						break;
					case SGS_GLOBAL:
						if ( !strcasecmp ( field, "CURRENTROUND" ) ) sscanf ( value, "%u", &global.currentround );
					...
					case SGS_PLAYERS:
						if ( !strcasecmp ( field, "PLAYERNUMBER" ) ) {
							...
							if ( ( player_idx > -1 ) && ( player_idx < env.numPermanentPlayers ) && ( player_count < MAXPLAYERS ) ) {
	...
	if ( env.campaign_rounds < 0. ) env.campaign_rounds = static_cast< double > ( env.rounds ) / 5.;
	if ( env.nextCampaignRound < 0. ) {
		env.nextCampaignRound = static_cast< double > ( env.rounds ) - env.campaign_rounds;
		while ( global.currentround < env.nextCampaignRound ) env.nextCampaignRound -= env.campaign_rounds;
	}

	// To ensure backwards compatibility, all players have to check
	// their opponents memory. Old save files do not provide any.
	for ( int32_t i = 0; i < env.numGamePlayers; ++i ) env.players[ i ]->checkOppMem();
...
bool Copy_Config_File() {
	...
	if ( !my_home_folder ) my_home_folder = xHere;
...
void scrollTextList ( TEXTBLOCK* lines ) {
	...
	eBackgroundTypes bgType   = env.dynamicMenuBg ? static_cast< eBackgroundTypes > ( rand() % BACKGROUND_COUNT ) : BACKGROUND_BLANK;
	...
	do {
		...
		if ( ++tOffset >= INT_MAX ) tOffset = 0;
		if ( moving ) scrollOffset--;
			...
			// not a special line, let's read some data
				...
				if ( ( FS_WEAPONS == file_stage ) && ( weapon_count < WEAPONS ) ) {
					if ( DS_NAME == data_stage )
						...
						sscanf (
							line,
							"%d %d %lf %lf %d %d %d %d %d %d %d %d %d %d %d %d %lf %d %lf %lf %lf %d %lf",
