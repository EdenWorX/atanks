...
void Create_Sky() {
	...
	if ( !env.custom_background || !env.sky ) {
		...
		generate_sky ( nullptr, sky_gradients[ global.cursky ], ( env.ditherGradients ? GENSKY_DITHERGRAD : 0 ) | ( env.detailedSky ? GENSKY_DETAILED : 0 ) );
	}
...
int Client_Cycle_Weapon ( PLAYER *my_player, int forward_or_back ) {
	...
	while ( !found ) {
		...
		if ( my_player->tank->cw < WEAPONS ) {
			if ( my_player->nm[ my_player->tank->cw ] ) found = true;
		} else // an item
		{
			if ( ( item[ my_player->tank->cw - WEAPONS ].selectable ) && ( my_player->ni[ my_player->tank->cw - WEAPONS ] ) ) found = true;
		}
	}
	return TRUE;
}
	...
	while ( !end_of_round ) {
			...
			if ( bytes_read > 0 ) {
				...
				else // not a special command, parse it
				{
					if ( Parse_Client_Data ( buffer ) ) {
						if ( game_stage < CLIENT_PLAYING ) game_stage++;
						...
						if ( game_stage < CLIENT_PLAYING ) {
							...
							if ( written < towrite )
								fprintf (
									stderr, "%s:%d: Warning: Only %d/%d bytes sent to server\n", __FILE__, __LINE__, written, towrite
								);
						...
						if ( fired ) {
							if ( ( global.client_player ) && ( global.client_player->tank ) ) {
								...
								if ( global.client_player->tank->cw < WEAPONS )
									SAFE_WRITE ( socket_number, "WEAPON %d", global.client_player->tank->cw )
								else
									SAFE_WRITE ( socket_number, "ITEM %d", global.client_player->tank->cw - WEAPONS )
							}
							...
							tank_position++;
							if ( tank_position >= env.numGamePlayers ) tank_position = 0;
						} else if ( game_stage == CLIENT_TANK_HEALTH ) {
							SAFE_WRITE ( socket_number, "HEALTH %d", tank_health )
							tank_health++;
							if ( tank_health >= env.numGamePlayers ) tank_health = 0;
		...
		while ( class_ < CLASS_COUNT ) {
			...
			while ( my_object ) {
				...
				if ( CLASS_EXPLOSION == class_ ) static_cast< EXPLOSION * > ( my_object )->explode();
				...
				if ( my_object->destroy ) {
					...
					my_object->update();
					delete my_object;
					if ( CLASS_TELEPORT == class_ ) time_clock = 2;
				}

				if ( ( CLASS_BEAM == class_ ) || ( CLASS_MISSILE == class_ ) || ( CLASS_EXPLOSION == class_ ) || ( CLASS_TELEPORT == class_ ) )
