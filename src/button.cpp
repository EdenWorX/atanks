 ...
 */
BUTTON::BUTTON ( int32_t left_, int32_t top_, BITMAP* bmp_, BITMAP* hover_, BITMAP* depressed_ )
	: bmp ( bmp_         ? bmp_
                : hover_     ? hover_
...
void BUTTON::draw() {
	...
	if ( text ) textout_centre_ex ( global.canvas, font, text, x3, y3 - ( text_height ( font ) / 2 ), text_only ? BLACK : WHITE, -1 );
