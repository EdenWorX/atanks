...
void FLOATTEXT::applyPhysics() {
	...
	if ( TS_HORIZONTAL == sway ) {
		...
		double rel_yv = static_cast< double >( sway - std::abs( y_dist ) ) / static_cast< double >( sway )
		              * SIGNd( y_dist ); // [-1:+1]
...
int32_t GetShadeColor( int32_t colour, bool do_lighten, int32_t bg_colour ) {
	...
	if ( bg_colour != PINK ) {
		...
		if ( ABSDISTANCE3( rb, gb, bb, rm, gm, bm ) < 0x20 ) {
			...
			rn = std::round(
				static_cast< double >( rn ) * ( rn > r ? std::max( s_mod, v_mod ) : std::min( s_mod, v_mod ) )
			);
			gn = std::round(
				static_cast< double >( gn ) * ( gn > g ? std::max( s_mod, v_mod ) : std::min( s_mod, v_mod ) )
			);
			bn = std::round(
				static_cast< double >( bn ) * ( bn > b ? std::max( s_mod, v_mod ) : std::min( s_mod, v_mod ) )
			);
			// Do not overflow:
			if ( rn > 255 ) rn = 255;
			if ( gn > 255 ) gn = 255;
