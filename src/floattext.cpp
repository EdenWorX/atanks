...
FLOATTEXT::FLOATTEXT (
	...
	: VIRTUAL_OBJECT(), color ( color_ ), is_fixed ( is_fixed_ ), is_pushed ( !is_fixed ), pos_x ( xpos ), pos_y ( ypos ) {
...
FLOATTEXT::~FLOATTEXT() {
	...
	if ( dim_cur.w > 0 ) {
		...
		if ( ( right > left ) && ( bottom > top ) ) global.make_bgupdate ( left, top, right - left, bottom - top );
	}
...
void FLOATTEXT::applyPhysics() {
	...
	if ( ( nullptr == text ) || ( dim_cur.w < 1 ) ) return;

	if ( TS_HORIZONTAL == sway ) {
		double x_dist = pos_x - x;
		double rel_xv = static_cast< double > ( sway - std::abs ( x_dist ) ) / static_cast< double > ( sway ) * SIGNd ( x_dist ); // [-1:+1]
		...
		double rel_yv = static_cast< double > ( sway - std::abs ( y_dist ) ) / static_cast< double > ( sway ) * SIGNd ( y_dist ); // [-1:+1]
	...
	if ( ( maxAge != -1 ) && ( ++age > maxAge ) ) destroy = true;
}
	...
	if ( is_fixed )
		...
		while ( curr ) {
			if ( this == curr )
				...
				// pushed.
				if ( curr_is_older || is_new ) this->push_down ( curr->overlaps_by ( this ), true );
...
#define SAFE_MAKECOL( r_, g_, b_ ) makecol ( r_ < 0 ? 0 : r_ > 255 ? 255 : r_, g_ < 0 ? 0 : g_ > 255 ? 255 : g_, b_ < 0 ? 0 : b_ > 255 ? 255 : b_ )

void FLOATTEXT::draw() {
	// Opt out early if there is no text to be drawn.
	if ( ( nullptr == text ) || !dim_cur.w ) return;

	// If the width is not known, yet, determine it
	if ( 1 > dim_cur.w ) {
		dim_cur.w  = text_length ( font, text ) + 3;
		dim_cur.w += dim_cur.w % 2;
		if ( !is_fixed ) check_pos ( true );
	} else if ( is_pushed )
	...
	if ( ( env.shadowedText || env.fadingText ) && !global.skippingComputerPlay ) {
		backCol = global.get_avg_bgcolor ( left, top, left + dim_cur.w, top + dim_cur.h, xv, yv );
	...
	if ( env.shadowedText && !global.skippingComputerPlay ) textout_ex ( global.canvas, font, text, left + 1, top + 1, shadeCol, -1 );
	textout_ex ( global.canvas, font, text, left, top, frontCol, -1 );
}

void FLOATTEXT::newRound() {
	if ( maxAge > 0 ) age = maxAge + 1;
}
...
int32_t FLOATTEXT::overlaps_by ( const FLOATTEXT* other ) {
	if ( other && ( other != this ) ) {
		...
		if ( this->is_fixed || other->is_fixed ) return 0; // Non of your business!

		// Otherwise only check vertical
		if ( ( ( other->dim_cur.x + other->dim_cur.w ) > dim_cur.x ) && ( ( dim_cur.x + dim_cur.w ) > other->dim_cur.x ) ) {
			int32_t this_y2  = dim_cur.y + dim_cur.h;
			int32_t other_y2 = other->dim_cur.y + other->dim_cur.h;

			if ( ( other_y2 > dim_cur.y ) && ( this_y2 > other->dim_cur.y ) ) {
...
void FLOATTEXT::push_down ( int32_t ydiff, bool is_new ) {
	if ( ydiff && !is_fixed ) {
		...
		// by only one pixel.
		int32_t push_by = is_new ? ( ydiff > 3 ? 3 : ydiff < -3 ? -3 : ydiff ) : ( ydiff > 1 ? 1 : ydiff < -1 ? -1 : ydiff );
...
void FLOATTEXT::set_text ( const char* text_ ) {
	...
	// clean up old text
	if ( text && old_len ) memset ( text, 0, old_len * sizeof ( char ) );
...
int32_t GetShadeColor ( int32_t colour, bool do_lighten, int32_t bg_colour ) {
	...
	hsv_to_rgb ( h, s * s_mod * ( v > 0.75 ? 0.5 : 1. ), v * v_mod * ( s > 0.75 ? 0.5 : 1. ), &rn, &gn, &bn );
	...
	if ( bg_colour != PINK ) {
		...
		if ( ABSDISTANCE3 ( rb, gb, bb, rm, gm, bm ) < 0x20 ) {
			...
			rn = std::round ( static_cast< double > ( rn ) * ( rn > r ? std::max ( s_mod, v_mod ) : std::min ( s_mod, v_mod ) ) );
			gn = std::round ( static_cast< double > ( gn ) * ( gn > g ? std::max ( s_mod, v_mod ) : std::min ( s_mod, v_mod ) ) );
			bn = std::round ( static_cast< double > ( bn ) * ( bn > b ? std::max ( s_mod, v_mod ) : std::min ( s_mod, v_mod ) ) );
			// Do not overflow:
			if ( rn > 255 ) rn = 255;
			if ( gn > 255 ) gn = 255;
			if ( bn > 255 ) bn = 255;
