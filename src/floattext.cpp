...
void CFloatText::draw() {
	...
	if ( ( env.shadowed_text || env.fading_text ) && !global.skipping_computer_play ) {
		...
		if ( env.fading_text && ( max_age > 0 ) && ( age >= ( max_age / 2 ) ) ) {
			...
			auto r   = ROUND( ( getr( frontCol ) * frontFade ) + ( getr( backCol ) * backFade ) );
			auto g   = ROUND( ( getg( frontCol ) * frontFade ) + ( getg( backCol ) * backFade ) );
			auto b   = ROUND( ( getb( frontCol ) * frontFade ) + ( getb( backCol ) * backFade ) );
		...
		if ( env.shadowed_text ) {
			...
			auto r   = ROUND( ( getr( shadeCol ) * shadeFade ) + ( getr( backCol ) * backFade ) );
			auto g   = ROUND( ( getg( shadeCol ) * shadeFade ) + ( getg( backCol ) * backFade ) );
			auto b   = ROUND( ( getb( shadeCol ) * shadeFade ) + ( getb( backCol ) * backFade ) );
...
void CFloatText::push_down( int32_t ydiff, bool is_new ) {
	if ( ydiff && !is_fixed ) {
		...
		// And when it is pushed down, it lowers its speed by 1% / 3%.
...
int32_t get_shade_color( int32_t colour, bool do_lighten, int32_t bg_colour ) {
	...
	hsv_to_rgb(
		h,
		static_cast< float >( s * s_mod * ( v > 0.75 ? 0.5 : 1. ) ),
		static_cast< float >( v * v_mod * ( s > 0.75 ? 0.5 : 1. ) ),
		&rn,
		&gn,
		&bn
	);
	...
	if ( bg_colour != PINK ) {
		...
		if ( ABSDISTANCE3( rb, gb, bb, rm, gm, bm ) < 0x20 ) {
			...
			// The middle between the color and its shade is too near
