...
void           generate_land ( LevelCreator* lcr, int32_t xoffset, int32_t heightx ) {
        ...
        int32_t land_type       = LAND_RANDOM == env.landType ? ( rand() % LAND_PLAIN ) + 1 : env.landType;
        ...
        for ( int32_t x = 0; lcr->can_work() && ( x < env.screenWidth ); ++x ) {
                ...
                if ( land_type != LAND_NONE ) surface = ( 1. + perlin2DPoint ( 1.0, smoothness, xoffset + x, 0, lambda, octaves ) ) / 2. * land_height;
        ...
        if ( WALL_WRAP == env.current_wallType ) {
                ...
                for ( int32_t x = 0; lcr->can_work() && ( x < length ); ++x ) {
                        ...
                        double  ratio_n = ( static_cast< double > ( x ) / static_cast< double > ( length ) / 2. ) + .5; // [n]ear: 50% at the wall, 100% at length.
        ...
        for ( int32_t x = 0; lcr->can_work() && ( x < env.screenWidth ); ++x ) {
                ...
                if ( env.detailedLandscape && ( LAND_NONE != land_type ) ) {
                        ...
                        for ( depth = 1; depth < env.screenHeight; depth++ ) {
                                depthStrip[ 1 ][ depth ] =
                                        ( 1. + perlin2DPoint ( 1.0, smoothness, xoffset + x, depth, lambda, octaves ) ) / 2. * land_height - ( env.screenHeight - depth );
                ...
                for ( int32_t y = 1; lcr->can_work() && ( y <= surface ); ++y ) {
                        ...
                        if ( env.detailedLandscape ) {
                                ...
                                double a1     = RAD2DEG ( atan2 ( depthStrip[ 0 ][ depth - 1 ] - depthStrip[ 1 ][ depth - 1 ], 1.0 ) ) + 180.;
                        ...
                        if ( env.ditherGradients ) offset += rand() % 10 - 5;
