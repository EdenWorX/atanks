...
public:
	...
	void operator() () {
		...
		while ( !doExit.load() ) {
			...
			updCondition.wait ( updLock, [ this ] { return ( doStart.load ( ATOMIC_READ ) || doExit.load ( ATOMIC_READ ) ); } );
			...
			while ( obj ) {
				...
				if ( ( false == has_action.load ( ATOMIC_READ ) )
				     && ( ( ( ( CLASS_BEAM == class_ ) || ( CLASS_MISSILE == class_ ) ) && static_cast< PHYSICAL_OBJECT* > ( obj )->isWeapon() )
				...
				if ( ( CLASS_DECOR_SMOKE == class_ ) && force_age ) static_cast< DECOR* > ( obj )->force_aging ( force_age );
				...
				if ( CLASS_TANK == class_ ) {
					...
					if ( !tmp_tank->destroy && ( env.maxFireTime > 0 ) && ( tmp_tank == curr_tank ) && ( HUMAN_PLAYER == tmp_tank->player->type )
					     && ( STAGE_AIM == global.stage ) && second_passed && tmp_tank->player->reduceClock() ) {
...
void game() {
	...
	while ( !done && ( global.get_command() != GLOBAL_COMMAND_QUIT ) ) {
		...
		if ( ( false == has_action.load ( ATOMIC_READ ) ) && !global.skippingComputerPlay && ( winner == WINNER_NO_WIN ) ) {
			do_naturals();
		...
		if ( !global.skippingComputerPlay && ( STAGE_SCOREBOARD == global.stage ) && ( global.get_command() != GLOBAL_COMMAND_QUIT ) ) draw_eor_scoreboard();
		...
		if ( !human_players && env.skipComputerPlay && !global.skippingComputerPlay && ( false == has_action.load() ) && ( global.numTanks > 1 )
		     && ( STAGE_SCOREBOARD > global.stage ) && ( WINNER_NO_WIN == winner ) ) {
...
static inline void change_wind_strength() {
	if ( !env.windvariation || !env.windstrength )
		...
		global.wind = global.lastwind + static_cast< double > ( rand() % ( env.windvariation * 100 ) ) / 100 - static_cast< double > ( env.windvariation ) / 2.;
...
static inline void draw_FPS_Counter() {
	...
	textprintf_ex ( global.canvas, font, FPS_pos + 1, MENUHEIGHT + 11, bc, -1, "% 4d FPS", FPS_last );
	textprintf_ex ( global.canvas, font, FPS_pos, MENUHEIGHT + 10, fc, -1, "% 4d FPS", FPS_last );
...
static inline void draw_objects ( AICore& aicore ) {
	...
	for ( int32_t class_ = 0; class_ < CLASS_COUNT; ++class_ ) {
		...
		while ( obj ) {
			...
			if ( ( false == has_deco.load ( ATOMIC_READ ) ) && ( ( CLASS_DECOR_DIRT == class_ ) || ( CLASS_DECOR_SMOKE == class_ ) ) )
...
static inline void draw_mini_scoreboard() {
	...
	for ( int i = 0; i < env.maxNumTanks; ++i ) {
		...
		if ( player ) {
			...
			textprintf_ex ( global.canvas, font, score_name_pos, line, color, -1, "%s", name );

			// Display money
			textprintf_ex ( global.canvas, font, score_money_pos + 1, line + 1, BLACK, -1, "$%s", money );
			textprintf_ex ( global.canvas, font, score_money_pos, line, color, -1, "$%s", money );
...
void draw_top_bar() {
	...
	if ( player ) {
		...
		textprintf_ex ( global.canvas, font, 150, y2, BLACK, -1, "%d", GET_DISP_ANGLE ( tank->a ) );
		...
		if ( tank->cw < WEAPONS ) {
			...
			textprintf_ex ( global.canvas, font, 180, y1, BLACK, -1, "%s: %d", item[ tank->cw - WEAPONS ].getName(), tank->player->ni[ tank->cw - WEAPONS ] );
		...
		textprintf_ex ( global.canvas, font, 386, y2, BLACK, -1, "%s: %d", env.ingame->Get_Line ( 21 ), tank->player->ni[ ITEM_FUEL ] );
	...
	textprintf_ex ( global.canvas, font, 500, y1, BLACK, -1, "%s %d/%d", env.ingame->Get_Line ( 12 ), env.rounds - global.currentround, env.rounds );

	// If a tank status is set, display it
	if ( global.tank_status[ 0 ] ) textprintf_ex ( global.canvas, font, 350, y3, global.tank_status_colour, -1, "%s", global.tank_status );
...
static inline bool explode_tanks() {
	...
	while ( tank ) {
		...
		if ( tank->destroy ) {
			...
			if ( do_vengeance && ( ( all_jedi_alive && ( TEAM_JEDI == tank->player->team ) ) || ( all_sith_alive && ( TEAM_SITH == tank->player->team ) ) ) )
				do_vengeance = false;
			...
			if ( ( tank->player ) && ( ( HUMAN_PLAYER == tank->player->type ) || ( NETWORK_CLIENT == tank->player->type ) ) ) --human_players;
...
static inline void fire_weapon() {
	assert ( ( STAGE_AIM == global.stage ) && " ERROR: fire_weapon() called, but not STAGE_AIM!" );
...
static inline void graph_bar ( int32_t x, int32_t y, int32_t col, int32_t actual, int32_t max_ ) {
	rect ( global.canvas, x, y, x + max_ + 2, y + 8, BLACK );
	rectfill ( global.canvas, x + 1, y + 1, x + 1 + actual, y + 7, col );
}

// Draws indication bar - centered
static inline void graph_bar_center ( int32_t x, int32_t y, int32_t col, int32_t actual, int32_t max_ ) {
	rect ( global.canvas, x, y, x + max_ + 2, y + 8, BLACK );
	rectfill ( global.canvas, x + 1 + max_ / 2, y + 1, x + 1 + actual + max_ / 2, y + 7, col );
}
...
static inline void init_new_round() {
	...
	for ( int32_t i = 0; i < env.numGamePlayers; ++i ) {
		...
		if ( ( env.players[ i ]->type == HUMAN_PLAYER ) || ( env.players[ i ]->type == NETWORK_CLIENT ) ) human_players++;
	}
...
static inline void set_level_settings ( LevelCreator* lcr ) {
	...
	if ( lcr->can_work() ) {
		...
		if ( !env.gfxData.sky_gradient_strips[ global.cursky ] ) {
			...
			env.gfxData.sky_gradient_strips[ global.cursky ] = create_gradient_strip ( sky_gradients[ global.cursky ], ( env.screenHeight - MENUHEIGHT ) );
	...
	if ( lcr->can_work() ) {
		...
		while ( !has_colours && lcr->can_work() ) {
			...
			for ( int32_t y = 2; lcr->can_work() && has_colours && ( y < max_y ); ++y ) {
				if ( colorDistance ( getpixel ( sky_gradient_strip, 0, bottom - y ), getpixel ( global.terrain, 0, env.screenHeight - y ) ) < min_dist )
					has_colours = false;
			}
	...
	if ( lcr->can_work() ) {
		...
		if ( !env.custom_background || !env.sky ) {
			...
			generate_sky ( lcr, sky_gradients[ global.cursky ], ( env.ditherGradients ? GENSKY_DITHERGRAD : 0 ) | ( env.detailedSky ? GENSKY_DETAILED : 0 ) );
		}
	}
		...
		while ( !sorted ) {
			...
			for ( int32_t index = 0; index < env.maxNumTanks - 1; ++index ) {
				...
				if ( env.turntype == TURN_HIGH ) {
					if ( global.order[ index ]->player->score < global.order[ index + 1 ]->player->score ) swap = true;
	...
	if ( ( winner != WINNER_NO_WIN ) && !global.demo_mode && !global.isCloseBtnPressed() ) {
		...
		if ( winner == WINNER_JEDI )
			...
			textprintf_centre_ex (
				global.canvas, font, env.halfWidth, y, env.players[ winner ]->color, -1, "%s: %s", env.ingame->Get_Line ( 47 ), env.players[ winner ]->getName()
			);
		...
		while ( score ) {
			...
			textprintf_right_ex ( global.canvas, font, scoStart + ( 3 * scoWidth ), y + ( z * lh ), score->diff < 0 ? RED : GREEN, -1, " %6d", score->diff );
...
sScore* sort_scores() {
	...
	for ( int32_t z = 0; z < env.numGamePlayers; z++ ) {
		...
		while ( curr && ( curr->score == scores[ z ].score ) && ( curr->diff == scores[ z ].diff ) && ( curr->kills > scores[ z ].kills ) ) curr = curr->next;

		// Walk to find a higher killed value:
		while ( curr && ( curr->score == scores[ z ].score ) && ( curr->diff == scores[ z ].diff ) && ( curr->kills == scores[ z ].kills )
		        && ( curr->killed < scores[ z ].killed ) )
			curr = curr->next;

		// Walk to find a higher name value:
		while ( curr && ( curr->score == scores[ z ].score ) && ( curr->diff == scores[ z ].diff ) && ( curr->kills == scores[ z ].kills )
		        && ( curr->killed == scores[ z ].killed ) && ( strcmp ( curr->name, scores[ z ].name ) < 0 ) )
			curr = curr->next;
