...
int32_t Menu::addButton (
	int32_t title_idx, const char* title_, int key_code, BITMAP* bmp, BITMAP* hover, BITMAP* released, bool text_only, int left, int top, int width, int height, int padding
) {
 ...
 **/
int32_t Menu::addColor ( int32_t* target, int32_t title_idx, int32_t left, int32_t top, int32_t width, int32_t height, int32_t show_size, int32_t padding ) {
	OptionItemBase* curr        = nullptr;
	bool            title_valid = is_title_idx_valid ( title_idx );
	...
	if ( target && title_valid ) {
		try {
			curr = new OptionItemColour (
				target, MenuTitleText[ menu_class ][ menu_lang ][ title_idx ], title_idx, menu_y + top, menu_x + left, width, height, padding, show_size
			);
		} catch ( std::bad_alloc& e ) {
 ...
 * @return Number of options in the menu after adding the button.
 **/
int32_t Menu::addMenu ( Menu* menu, int32_t title_idx, int32_t color, int32_t left, int32_t top, int32_t width, int32_t height, int32_t padding ) {
	OptionItemBase* curr        = nullptr;
	bool            title_valid = is_title_idx_valid ( title_idx );
	...
	if ( menu && title_valid ) {
		try {
			curr = new OptionItemMenu (
				menu, MenuTitleText[ menu_class ][ menu_lang ][ title_idx ], title_idx, color, menu_y + top, menu_x + left, width, height, padding
			);
		} catch ( std::bad_alloc& e ) {
	...
	if ( player && action_ ) {
		try {
			curr = new OptionItemPlayer (
				player,
				action_,
				title_valid ? MenuTitleText[ menu_class ][ menu_lang ][ title_idx ] : nullptr, // The ctor uses the player name if nullptr.
				title_idx,
				menu_y + top,
				menu_x + left,
				width,
 ...
 * @param[in] padding Distance between title and display.
 **/
int32_t Menu::addText ( char* target, int32_t title_idx, uint32_t max_len, int32_t color, const char* format, int left, int top, int width, int height, int padding ) {
	OptionItemBase* curr        = nullptr;
	bool            title_valid = is_title_idx_valid ( title_idx );
...
int32_t Menu::addToggle ( bool* target, const char* title_, int32_t color, int left, int top, int width, int height, int padding ) {
	...
	if ( target && title_ ) {
		try {
			curr = new OptionItem< bool, uint32_t > (
				target, nullptr, ET_TOGGLE, title_, -1, nullptr, color, TC_NONE, 0, 0, 0, nullptr, menu_y + top, menu_x + left, width, height, padding, nullptr
			);
		} catch ( std::bad_alloc& e ) {
			cerr << __FUNCTION__ << " : failed to allocate new TOGGLE OptionItem\n";
			cerr << " [" << e.what() << "]" << endl;
...
int32_t Menu::operator() () {
	...
	while ( -1 == key_code ) {
		...
		if ( mlb_is_released && mlb_is_pressed )
			...
			if ( ( --mouse_clock > 0 ) || ( ( ET_VALUE != last_clicked ) && ( ET_COLOR != last_clicked ) ) ) mlb_is_pressed = false;
		}
		...
		// if held and the left button is released.
			...
			if ( ( --mouse_clock > 0 ) || ( ( ET_VALUE != last_clicked ) && ( ET_COLOR != last_clicked ) ) ) mrb_is_pressed = false;
		}
		...
		if ( !mouse_clock ) {
			if ( ( ET_VALUE == last_clicked ) || ( ET_COLOR == last_clicked ) ) {
				if ( MOUSE_DELAY_REDUCT == ++mouse_round ) {
					mouse_round = 0;
					if ( ( ET_COLOR == last_clicked ) || ( ++mouse_reduct >= MOUSE_RELEASE_DELAY ) ) mouse_reduct = MOUSE_RELEASE_DELAY - 1;
				}
			} else
				mouse_reduct = 0;
		...
		int32_t event = mlb_is_pressed || mrb_is_pressed ? selectClicked ( mlb_x, mlb_y ) : 0;
...
void Menu::setTexts ( OptionItemBase* item, const char** texts, eTextClass text_class ) {
	assert ( item && ( texts || ( TC_FREETEXT != text_class ) ) && ( TC_NONE != text_class ) && "ERROR: This does not fit at all!" );
	if ( item ) {
...
bool display_tank_desc ( int32_t* tanknum, int32_t x, int32_t y ) {
	...
	global.make_update ( x, y, text_x + text_length ( font, tank_text ) - x, tank_y + turr_off_y + tank_off_y - y );
