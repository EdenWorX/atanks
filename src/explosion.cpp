...
EXPLOSION::EXPLOSION ( PLAYER* player_, double x_, double y_, double xv_, double yv_, int32_t type, bool is_weapon )
	: PHYSICAL_OBJECT ( is_weapon ), impact_xv ( xv_ ), impact_yv ( yv_ ) {
	xv = xv_;
	yv = yv_;
	...
	if ( ( ( SHAPED_CHARGE <= weapType ) && ( CUTTER >= weapType ) ) || ( DRILLER == weapType ) ) {
		// Uses FlameFront
		explo_w  = FLAME_W;
		explo_h  = FLAME_H;
		centre_x = FLAME_CX;
	...
	if ( ( weapType >= DIRT_BALL ) && ( weapType <= SMALL_DIRT_SPREAD ) ) {
		if ( y < minHeightDirt ) y = minHeightDirt;
	...
	if ( ( NAPALM_JELLY == weapType ) || ( ( weapType >= RIOT_BOMB ) && ( weapType <= CLUSTER_MIRV ) ) ) {
		...
		if ( ( NAPALM_JELLY == weapType ) || ( weapType >= DIRT_BALL ) ) apply_damage = false;
	...
	if ( ( ( weapType >= SML_LAZER ) && ( weapType <= LRG_LAZER ) ) || ( ( weapType >= SML_LIGHTNING ) && ( weapType <= LRG_LIGHTNING ) ) ) {
		apply_damage = false;
		etime        = 0;
	...
	if ( NAPALM_JELLY == weapType ) curFrame = ( ( rand() % ( 2 * env.frames_per_second ) ) - env.frames_per_second ) / etime;
...
EXPLOSION::~EXPLOSION() {
	...
	if ( ( TREMOR <= weapType ) && ( TECTONIC >= weapType ) ) global.unlockLandSlide ( dim_cur.x, dim_cur.x + dim_cur.w );
...
void EXPLOSION::applyPhysics() {
	if ( NAPALM_JELLY == weapType ) {
		...
		double blobSize = radius - ( static_cast< double > ( curFrame ) / static_cast< double > ( EXPLOSIONFRAMES ) * radius ) + 1.;
		...
		if ( can_move && ( y < ( env.screenHeight - 1 ) ) ) {
			if ( PINK != getpixel ( global.terrain, x, y + 1 ) ) can_move = false;
		}
		...
		while ( lt ) {
			...
			if ( !lt->destroy && lt->isInEllipse ( x, y, blobSize, blobSize, in_rate_x, in_rate_y ) ) {
				...
				if ( full_rate < 0.5 ) full_rate = 0.5;
				// Apply damage, but do it per frame
				lt->addDamage (
					player,
					( static_cast< double > ( damage ) * damage_mod * full_rate * ( player ? player->damageMultiplier : 1. )
...
void EXPLOSION::draw() {
	if ( ( curFrame > 1 ) && ( curFrame <= ( EXPLOSIONFRAMES + 1 ) ) && ( NAPALM_JELLY != weapType ) ) {
		...
		switch ( weapType ) {
			...
			case CUTTER:
				...
				setUpdateArea ( x - radius - 1, y - ( radius / 20 ) - 1, ( radius + 1 ) * 2, ( ( radius / 20 ) + 1 ) * 2 );
				break;
			case DRILLER:
				rotate_scaled_sprite (
					global.canvas,
				...
				setUpdateArea ( x - ( radius / 20 ) - 1, y - radius - 1, ( ( radius / 20 ) + 1 ) * 2, ( radius + 1 ) * 2 );
				break;
			case TREMOR:
			case SHOCKWAVE:
			case TECTONIC:
				if ( curFrame <= ( EXPLODEFRAMES + 1 ) ) {
					double tst_width =
						static_cast< double > ( curFrame ) / static_cast< double > ( EXPLODEFRAMES ) * static_cast< double > ( radius ) / 3.;
					drawFracture ( x, y, angle, static_cast< int32_t > ( .75 + tst_width ), radius * 1.75, ( weapType - TREMOR + 1 ) * 3, 0 );
			...
			default:
				if ( ( weapType <= LAST_EXPLOSIVE ) || ( weapType >= WEAPONS ) || ( weapType == PERCENT_BOMB ) ) {
					...
					if ( !peaked && ( weapType >= DIRT_BALL ) && ( weapType <= SMALL_DIRT_SPREAD ) ) {
						...
						masked_blit ( global.terrain, tmp, x - rad, y - rad, 0, 0, rad * 2, rad * 2 );

						// blit back exploded terrain
						masked_blit ( tmp, global.terrain, 0, 0, x - rad, y - rad, rad * 2, rad * 2 );
						destroy_bitmap ( tmp );
		...
		int32_t blobSize =
			curFrame > 0 ? static_cast< int32_t > ( radius - ( static_cast< double > ( curFrame ) / static_cast< double > ( EXPLOSIONFRAMES ) * radius ) ) + 1
				     : radius;
	...
	///-------------------------------------------------------------------
	if ( !hasThrown && ( curFrame > 1 ) && ( curFrame <= EXPLOSIONFRAMES ) && ( ( weapType <= TECTONIC ) || ( weapType > REDUCER ) )
	     && ( NAPALM_JELLY != weapType ) ) {

		do_throw();

		// The tremor types do not need clearing!
		if ( ( TREMOR > weapType ) || ( TECTONIC < weapType ) ) do_clear();
	}
}
	...
	if ( recurseDepth < maxRecurse ) {
		for ( int32_t branchCount = 0; branchCount < 3; ++branchCount ) {
			if ( branchCount || ( Noise ( x + y + branchCount ) < 0 ) ) {
				...
				drawFracture ( x3, y3, newAngle, width / reduction, segmentLength / reduction, maxRecurse, recurseDepth + 1 );
			}
		}
	}
...
void EXPLOSION::explode() {
	...
	if ( curFrame > ( EXPLOSIONFRAMES + 1 ) ) destroy = true;
			...
			while ( lt ) {
				...
				if ( dmg > 0. ) {
					if ( PERCENT_BOMB == weapType )
						...
						int32_t     amount          = lt->player->money <= max_amount ? lt->player->money : max_amount; // you have?
						...
						if ( !global.skippingComputerPlay ) {
							...
							try {
								new FLOATTEXT (
									the_money, lt->x, lt->y - 30, .0, -.5, RED, CENTRE, env.swayingText ? TS_HORIZONTAL : TS_NO_SWAY, 200, false
								);
								if ( global.stage < STAGE_SCOREBOARD ) global.updateMenu = true;
							} catch ( std::exception& e ) {
...
void EXPLOSION::do_throw() {
	...
	if ( !hasThrown && ( global.skippingComputerPlay || ( hasDebris >= maxDebris ) || ( curFrame > maxFrame ) ) ) hasThrown = true;

	// Early out if this is already done or no further deco is allowed
	if ( hasThrown || global.hasTooMuchDeco ) return;
	...
	do {
		...
		xpos = ( seek_area ? 1 == seek_area ? minX // left
		                                    : x    // right
		                   : x - ( xrad / 2 )      // centre
		       )
		     + ( rand() % xrad );
		...
		if ( ( ypos < y ) && ( maxY > ypos ) && checkPixelsBetweenTwoPoints ( &xpos, &ypos, xpos, maxY, 0.0, nullptr ) ) {
			...
			// Blit in terrain
			blit ( global.terrain, deb_item->bmp, left, top, 0, 0, diameter + 1, diameter + 1 );

			// Blit in meteor if needed
			if ( isMeteor && meteor && met_item )
				blit ( meteor, met_item->bmp, left % ( meteor->w - diameter ), top % ( meteor->h - diameter ), 0, 0, diameter + 1, diameter + 1 );
			...
			if ( std::abs ( dxv ) > max_x_vel ) dxv = SIGNd ( dxv ) * max_x_vel;
			if ( dyv < max_y_vel ) dyv = max_y_vel;
			...
			try {
				...
				if ( met_item ) global.free_debris_item ( met_item );
			}
	...
	if ( ( rad >= radius ) || ( hasDebris >= maxDebris ) ) hasThrown = true;
}
...
double get_hit_damage ( TANK* tank, weaponType type, int32_t hit_x, int32_t hit_y ) {
	if ( ( nullptr == tank ) || ( tank->destroy ) ) return 0.;

	double weap_rad = type < WEAPONS ? weapon[ type ].radius : naturals[ type - WEAPONS ].radius;
	double xrad     = weap_rad;
	double yrad     = weap_rad;

	// Adapt x-/y-radius for the driller and the shaped weapons
	if ( DRILLER == type ) xrad /= 20;
	if ( ( SHAPED_CHARGE <= type ) && ( CUTTER >= type ) ) yrad /= 20;
	...
	if ( tank->isInEllipse ( hit_x, hit_y, xrad, yrad, in_rate_x, in_rate_y ) ) {
		...
		else if ( ( REDUCER == type ) && ( tank->player->damageMultiplier > 0.1 ) )
		...
		// deal no damage:
			...
			dmg = type < WEAPONS ? weapon[ type ].damage : naturals[ type - WEAPONS ].damage;

			// Some weapons have minimum rates on axis ratings
			if ( DRILLER == type ) {
				// The driller has its force focused vertically:
				if ( in_rate_y < 0.95 ) in_rate_y = 0.95;
			} else if ( ( SHAPED_CHARGE <= type ) && ( CUTTER >= type ) ) {
				// The shaped ones have their force on the horizontal axis
				if ( in_rate_x < 0.95 ) in_rate_x = 0.95;
			} else if ( ( TREMOR <= type ) && ( TECTONIC >= type ) ) {
				if ( in_rate_x < 0.25 ) in_rate_x = 0.25;
				if ( in_rate_y < 0.25 ) in_rate_y = 0.25;
			}
