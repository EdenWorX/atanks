...
#  define atanks_snprintf( target, char_count, fmt, ... ) _snprintf_s( target, char_count + 1, char_count, fmt, __VA_ARGS__ )
#  define atanks_tzset()                                  _tzset()
#  define atanks_localtime( tm_p, time_p )                localtime_s( tm_p, time_p )
#else
#  define atanks_snprintf( target, char_count, fmt, ... ) snprintf( target, char_count, fmt, __VA_ARGS__ )
