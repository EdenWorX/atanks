...
static void build_Physics ( Menu &mPhysics, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY );
static void build_Weather ( Menu &mWeather, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY );
static void build_Graphics ( Menu &mGraphics, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY );
static void build_Money ( Menu &mMoney, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY );
static void build_Network ( Menu &mNetwork, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY );
static void build_Sound ( Menu &mSound, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY );
...
void    drawMenuBackground ( eBackgroundTypes backType, int32_t tOffset, int32_t numItems ) {
        ...
        for ( int32_t tCount = 0; tCount < numItems; tCount++ ) {
                int32_t radius = static_cast< int32_t > ( ( perlin1DPoint ( 1.0, 5, ( tOffset * 0.0333 ) + tCount + 423346, 0.5, 8 ) + 1.1 ) * 20 ); // [0.1;2.1]
                int32_t xpos = env.halfWidth + static_cast< int32_t > ( perlin1DPoint ( 1.0, 3, ( tOffset * 0.0166 ) + tCount + 232662, 0.3, 3 ) * ( 299 - radius ) );
                int32_t ypos =
                        env.halfHeight
                        + static_cast< int32_t > (
                                perlin1DPoint ( 1.0, 2, ( tOffset * 0.0175 ) + tCount + 42397, 0.3, 3 ) * ( env.halfHeight - env.menuBeginY - radius - 1 )
                        );
                switch ( backType ) {
                        ...
                        case BACKGROUND_LINE:
                                rectfill ( global.canvas, xpos - radius / 2, env.menuBeginY + 1, xpos + radius / 2, env.menuEndY - 1, LIME_GREEN );
                                break;
...
void editPlayers() {
	...
	// Add "back" button
	menu.addButton (
		2, nullptr, KEY_ESC, env.misc[ 7 ], nullptr, env.misc[ 8 ], false, menuMid - ( env.misc[ 7 ]->w / 2 ), menuHeight - btnHeight - itemPadding - 2, 0, 0, 2
	);
	...
	while ( ( KEY_ESC != optionsRetVal ) && !global.isCloseBtnPressed() ) {
		...
		if ( PE_CONFIRM_EDIT & optionsRetVal )
			...
			if ( player_new ) {
				...
				menu.addMenu ( &env.allPlayers[ env.numPermanentPlayers - 1 ], edit_player, -1, 0, 0, max_width + 15, itemHeight, itemPadding );
...
void optionsMenu() {
	...
	mReset.addButton ( 1, nullptr, RO_RESET, env.misc[ 7 ], nullptr, env.misc[ 8 ], false, menuMid + 50, menuHeight - btnHeight - 6, 0, 0, itemPadding );
	mReset.addButton (
		2, nullptr, RO_BACK, env.misc[ 7 ], nullptr, env.misc[ 8 ], false, menuMid - env.misc[ 7 ]->w - 50, menuHeight - btnHeight - 6, 0, 0, itemPadding
	);
	...
	// "Landscape"
	mMain.addValue (
		&env.landType, idx++, nullptr, WHITE, TC_LANDTYPE, static_cast< int32_t > ( LAND_NONE ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;

	// "Turn Order"
	mMain.addValue (
		&env.turntype, idx++, nullptr, WHITE, TC_TURNTYPE, static_cast< int32_t > ( TURN_SIMUL ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;

	// "Skip AI-only play"
	mMain.addValue (
		&env.skipComputerPlay, idx++, nullptr, WHITE, TC_SKIPTYPE, static_cast< int32_t > ( SKIP_HUMANS_DEAD ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;
	...
	// "Language"
	mMain.addValue (
		&env.language, switch_language, idx++, nullptr, WHITE, TC_LANGUAGE, static_cast< int32_t > ( EL_ITALIAN ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);

	// "Back"
	mMain.addButton (
		idx, nullptr, KEY_ESC, env.misc[ 7 ], nullptr, env.misc[ 8 ], false, menuMid - ( env.misc[ 7 ]->w / 2 ), menuHeight - btnHeight - itemPadding - 2, 0, 0, 2
	);
...
int32_t selectPlayers() {
	...
	if ( ( saved_game_names ) && ( number_saved_games ) ) {
		...
		menu.addValue (
			&env.saved_gameindex, idx++, env.saved_game_list, BLACK, TC_FREETEXT, number_saved_games - 1, menuMid - ( itemWidth / 2 ), itemY, itemWidth, itemHeight, itemPadding
		);
	...
	int32_t first_idx  = menu.addToggle ( &env.campaign_mode, idx++, WHITE, menuMid + 25, itemY, 100, itemHeight + 5, itemPadding );
	...
	menu.addButton (
		idx + 1, nullptr, KEY_ESC, env.misc[ 7 ], nullptr, env.misc[ 8 ], false, menuMid - env.misc[ 7 ]->w - 25, menuHeight - btnHeight - itemPadding - 2, 0, 0, 2
	);
	menu.addButton ( idx, nullptr, KEY_ENTER, env.misc[ 7 ], nullptr, env.misc[ 8 ], false, menuMid + 25, menuHeight - btnHeight - itemPadding - 2, 0, 0, 2 );
	...
	} while ( ( KEY_ESC != optionsRetVal ) && ( MRC_Play_Game != optionsRetVal ) && ( MRC_Load_Game != optionsRetVal ) && !global.isCloseBtnPressed() );
...
static void build_Physics ( Menu &mPhysics, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY ) {
	...
	// "Land Slide"
	mPhysics.addValue (
		&env.landSlideType, idx++, nullptr, WHITE, TC_LANDSLIDE, static_cast< int32_t > ( SLIDE_CARTOON ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;
	...
	// "Wall Type"
	mPhysics.addValue (
		&env.wallType, idx++, nullptr, WHITE, TC_WALLTYPE, static_cast< int32_t > ( WALL_RANDOM ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;
	...
	// "Boxed Mode",
	mPhysics.addValue (
		&env.boxedMode, idx++, nullptr, WHITE, TC_OFFONRANDOM, static_cast< int32_t > ( BM_RANDOM ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;
	...
	mPhysics.addValue (
		&env.do_box_wrap, idx++, nullptr, WHITE, TC_OFFON, static_cast< int32_t > ( BM_ON ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;
	...
	// "Violent Death"
	mPhysics.addValue (
		&env.violent_death, idx++, nullptr, WHITE, TC_LIGHTNING, static_cast< int32_t > ( VD_HEAVY ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;
	...
	// "Explosion Debris"
	mPhysics.addValue ( &env.debris_level, idx++, nullptr, WHITE, TC_LIGHTNING, 3, menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
...
static void build_Weather ( Menu &mWeather, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY ) {
	...
	// "Falling Dirt"
	mWeather.addValue ( &env.falling_dirt_balls, idx++, nullptr, WHITE, TC_METEOR, 3, menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;

	// "Laser Satellite"
	mWeather.addValue (
		&env.satellite, idx++, nullptr, WHITE, TC_SATELLITE, static_cast< int32_t > ( SL_SUPER ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;
...
static void build_Graphics ( Menu &mGraphics, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY ) {
	...
	// "Dithering"
	mGraphics.addValue ( &env.ditherGradients, idx++, nullptr, WHITE, TC_OFFON, 1, menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;

	// "Detailed Land"
	mGraphics.addValue ( &env.detailedLandscape, idx++, nullptr, WHITE, TC_OFFON, 1, menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;
	...
	// "Colour Theme"
	mGraphics.addValue (
		&env.colourTheme, idx++, nullptr, WHITE, TC_COLOUR, static_cast< int32_t > ( CT_CRISPY ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;

	// "Screen Width"
	mGraphics.addValue ( &env.temp_screenWidth, idx++, WHITE, 800, 2000, 100, "%d", menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;

	// "Screen Height"
	mGraphics.addValue ( &env.temp_screenHeight, idx++, WHITE, 600, 1400, 100, "%d", menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;
	...
	// "Custom Background"
	mGraphics.addValue ( &env.custom_background, idx++, nullptr, WHITE, TC_OFFON, 1, menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;

	// "Show AI Feedback"
	mGraphics.addValue ( &env.showAIFeedback, idx++, nullptr, WHITE, TC_OFFON, 1, menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;
...
static void build_Money ( Menu &mMoney, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY ) {
	...
	// "Round Win Bonus"
	mMoney.addValue ( &env.scoreRoundWinBonus, idx++, WHITE, 0, 50000, 5000, "%d", menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;
	...
	mMoney.addValue ( &env.scoreUnitDestroyBonus, idx++, WHITE, 0, 20000, 2500, "%d", menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;

	// "Tank Self-Destruction Penalty"
	mMoney.addValue ( &env.scoreUnitSelfDestroy, idx++, WHITE, 0, 20000, 2500, "%d", menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;
...
static void build_Network ( Menu &mNetwork, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY ) {
	...
	// "Check Updates"
	mNetwork.addValue ( &env.check_for_updates, idx++, nullptr, WHITE, TC_OFFON, 1, menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;

	// "Networking"
	mNetwork.addValue ( &env.network_enabled, idx++, nullptr, WHITE, TC_OFFON, 1, menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;

	// "Listen Port"
	mNetwork.addValue ( &env.network_port, idx++, WHITE, 10645, 64645, 1000, "%d", menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
	itemY += itemFullHeight;
...
static void build_Sound ( Menu &mSound, int32_t menuMid, int32_t itemWidth, int32_t itemHeight, int32_t itemPadding, int32_t itemY ) {
	...
	// "Sound Driver"
	mSound.addValue (
		&env.sound_driver, idx++, nullptr, WHITE, TC_SOUNDDRIVER, static_cast< int32_t > ( SD_JACK ), menuMid - 50, itemY, itemWidth, itemHeight, itemPadding
	);
	itemY += itemFullHeight;
	...
	// "Volume Faktor"
	mSound.addValue ( &env.volume_factor, idx++, WHITE, 0, MAX_VOLUME_FACTOR, 1, "%d", menuMid - 50, itemY, itemWidth, itemHeight, itemPadding );
